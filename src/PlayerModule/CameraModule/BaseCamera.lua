...
--[[
Modifications:
Touch devices should be able to adjust their camera sensitivity using and modifying
a number attribute of this ModuleScript named "TouchCameraSensitivity"
]]--
...
function BaseCamera:OnTouchChanged(input, processed)
	...
	if self.numUnsunkTouches == 1 then
		if self.fingerTouches[input] == false then
			...
			-- modification: use the TouchCameraSensitivity attribute
			local delta = input.Position - self.lastPos
			local touchCamSensitivity = script:GetAttribute("TouchCameraSensitivity") or 1
			delta = Vector2.new(delta.X * touchCamSensitivity, delta.Y * touchCamSensitivity * UserGameSettings:GetCameraYInvertValue())
