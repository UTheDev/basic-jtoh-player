...
--[[
	...
	@class ClientObjectPlayer
]]
--
local ClientObjectPlayer = {}

ClientObjectPlayer = {}
ClientObjectPlayer.__index = ClientObjectPlayer

--[[
	An index of the script repository for quick referencing
]]
--
ClientObjectPlayer.scriptRepoIndex = {}

--[[
	Creates a new ClientObjectPlayer runner

	@param clientObjectFolder Folder -- The folder containg the client objects
	@param coFolderParent Instance -- The instance to use as the client object folder's parent
]]
--
function ClientObjectPlayer.new(clientObjectFolder: Folder, coFolderParent: Instance)
	local self = {}
	...
	return setmetatable(self, ClientObjectPlayer)
end

--[[
	@return Instance -- Whether or not the object represented by "obj" is a ClientObject value
]]
--
function ClientObjectPlayer.isClientObjectValue(obj: Instance)
...
@param runningObject {stop: () -> ()?} -- The client object table
]]
--
function ClientObjectPlayer.stopClientObject(runningObject: { stop: () -> ()? })
...
--
function ClientObjectPlayer.indexCOScriptRepo()
	local indexTable = {}
	...
	ClientObjectPlayer.scriptRepoIndex = indexTable
end
...
coSession <{}> - The current ClientObjectPlayer
```
]]
--
function ClientObjectPlayer:runScript(coValue: ValueBase?, coFunc: (ref: ValueBase?, coSession: {}) -> {}?)
	-- run the function
	task.spawn(function()
		...
		if typeof(result) == "table" then
			if self.isRunning then
				...
				ClientObjectPlayer.stopClientObject(result)
			end
		end
	end)
end
...
local function getRepoScriptFromStringValue(val: StringValue)
	...
	if val:GetAttribute("IsAbsolutePath") == true then
		...
		repoModule = ClientObjectPlayer.scriptRepoIndex[val.Value]
	end
...
@param inst Instance -- The instance to apply the behavior to
]]
--
function ClientObjectPlayer:applyPart(inst: Instance)
	if inst:IsA("ModuleScript") then
		if inst.Name == "ChangeLighting" then
			for p, l in pairs(require(inst)) do
				if not origlighting[p] then
					origlighting[p] = game.Lighting[p]
				end
				game.Lighting[p] = l
			end
		elseif inst.Name == "ClientObjectScript" then
			-- handle client object script
			self:runScript(nil, require(inst))
		end
	elseif inst:IsA("BasePart") then
		if inst.Name == "LightingChanger" or inst:FindFirstChild("Invisible") then
			inst.Transparency = 1
		end

		local collisionGroupVal = inst:FindFirstChild("SetCollisionGroup")
		if collisionGroupVal then
			PhysicsService:SetPartCollisionGroup(inst, collisionGroupVal.Value)
		end
	elseif inst:IsA("StringValue") then
		...
		-- for them to work
		if inst.Name == "RunRepoScript" then
			local scr = getRepoScriptFromStringValue(inst) --ClientObjectScriptRepo:FindFirstChild(w.Value)
			if scr then
				scr = scr:Clone()
				scr.Name = "RepoScript"
				scr.Parent = inst.Parent

				self:runScript(inst, require(scr))
			end
		elseif inst.Name == "ReferenceCOScript" then
			local scr = getRepoScriptFromStringValue(inst)
			if scr then
...
--
function ClientObjectPlayer:run()
	if self.isRunning == false then
		...
		for i, v in pairs(clientObjectFolder:GetDescendants()) do
			if ClientObjectPlayer.isClientObjectValue(v) then
		...
		for i, v in pairs(clientObjectFolder:GetDescendants()) do
			--if not ClientObjectPlayer.isClientObjectValue(v) then
			--local clone = v:Clone();
...
--
function ClientObjectPlayer:stop()
	if self.isRunning then
		self.isRunning = false
		...
		for i, v in pairs(self.runningObjects) do
			ClientObjectPlayer.stopClientObject(v)
		end
		self.runningObjects = {}
...
ClientObjectPlayer.indexCOScriptRepo()

return ClientObjectPlayer
